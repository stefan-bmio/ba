...
def load_examples():
    ...
    with tf.compat.v1.name_scope("load_images"):
        ...
        assertion = tf.compat.v1.assert_equal(tf.compat.v1.shape(raw_input)[2], 3,
    ...
    def transform(image):
        ...
        offset = tf.compat.v1.cast(
            tf.compat.v1.floor(tf.compat.v1.random_uniform([2], 0, a.scale_size - CROP_SIZE + 1, seed=seed)),
...
def create_model(inputs, targets):
    ...
    with tf.compat.v1.name_scope("discriminator_loss"):
        ...
        # predict_real => 1
        # predict_fake => 0
        discrim_loss = tf.compat.v1.reduce_mean(
            -(tf.compat.v1.log(predict_real + EPS) + tf.compat.v1.log(1 - predict_fake + EPS)))
...
def main():
    ...
    if a.mode == "export":
        ...
        input_image = tf.compat.v1.cond(tf.compat.v1.equal(tf.compat.v1.shape(input_image)[2], 4),
    ...
    with tf.compat.v1.name_scope("encode_images"):
        display_fetches = {
            ...
            "inputs": tf.compat.v1.map_fn(tf.compat.v1.image.encode_png, converted_inputs, dtype=tf.compat.v1.string,
                                          name="input_pngs"),
            "targets": tf.compat.v1.map_fn(tf.compat.v1.image.encode_png, converted_targets, dtype=tf.compat.v1.string,
                                           name="target_pngs"),
            "outputs": tf.compat.v1.map_fn(tf.compat.v1.image.encode_png, converted_outputs, dtype=tf.compat.v1.string,
                                           name="output_pngs"),
        }
    ...
    with tf.compat.v1.name_scope("predict_real_summary"):
        tf.compat.v1.summary.image("predict_real",
                                   tf.compat.v1.image.convert_image_dtype(model.predict_real, dtype=tf.compat.v1.uint8))

    with tf.compat.v1.name_scope("predict_fake_summary"):
        tf.compat.v1.summary.image("predict_fake",
                                   tf.compat.v1.image.convert_image_dtype(model.predict_fake, dtype=tf.compat.v1.uint8))
    ...
    with tf.compat.v1.name_scope("parameter_count"):
        parameter_count = tf.compat.v1.reduce_sum(
            [tf.compat.v1.reduce_prod(tf.compat.v1.shape(v)) for v in tf.compat.v1.trainable_variables()])
