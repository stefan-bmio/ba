...
def preprocess_lab(lab):
    with tf.compat.v1.name_scope("preprocess_lab"):
        L_chan, a_chan, b_chan = tf.compat.v1.unstack(lab, axis=2)
...
def deprocess_lab(L_chan, a_chan, b_chan):
    with tf.compat.v1.name_scope("deprocess_lab"):
        ...
        return tf.compat.v1.stack([(L_chan + 1) / 2 * 100, a_chan * 110, b_chan * 110], axis=3)


def augment(image, brightness):
    # (a, b) color channels, combine with L channel and convert to rgb
    a_chan, b_chan = tf.compat.v1.unstack(image, axis=3)
    L_chan = tf.compat.v1.squeeze(brightness, axis=3)
...
def lrelu(x, a):
    with tf.compat.v1.name_scope("lrelu"):
        ...
        return (0.5 * (1 + a)) * x + (0.5 * (1 - a)) * tf.compat.v1.abs(x)
...
def check_image(image):
    assertion = tf.compat.v1.assert_equal(tf.compat.v1.shape(image)[-1], 3, message="image must have 3 color channels")
    with tf.compat.v1.control_dependencies([assertion]):
        image = tf.compat.v1.identity(image)
...
def load_examples():
    ...
    decode = tf.compat.v1.image.decode_jpeg
    if len(input_paths) == 0:
        input_paths = glob.glob(os.path.join(a.input_dir, "*.png"))
        decode = tf.compat.v1.image.decode_png
    ...
    with tf.compat.v1.name_scope("load_images"):
        ...
        raw_input = tf.compat.v1.image.convert_image_dtype(raw_input, dtype=tf.compat.v1.float32)

        assertion = tf.compat.v1.assert_equal(tf.compat.v1.shape(raw_input)[2], 3, message="image does not have 3 channels")
        with tf.compat.v1.control_dependencies([assertion]):
            raw_input = tf.compat.v1.identity(raw_input)
        ...
        if a.lab_colorization:
            ...
            a_images = tf.compat.v1.expand_dims(L_chan, axis=2)
    ...
    def transform(image):
        r = image
        if a.flip:
            r = tf.compat.v1.image.random_flip_left_right(r, seed=seed)
...
def create_model(inputs, targets):
    def create_discriminator(discrim_inputs, discrim_targets):
        ...
        with tf.compat.v1.variable_scope("layer_%d" % (len(layers) + 1)):
            ...
            output = tf.compat.v1.sigmoid(convolved)
            layers.append(output)
    ...
    with tf.compat.v1.name_scope("discriminator_loss"):
        ...
        # predict_real => 1
        # predict_fake => 0
        discrim_loss = tf.compat.v1.reduce_mean(-(tf.compat.v1.log(predict_real + EPS) + tf.compat.v1.log(1 - predict_fake + EPS)))

    with tf.compat.v1.name_scope("generator_loss"):
        # predict_fake => 1
        # abs(targets - outputs) => 0
        gen_loss_GAN = tf.compat.v1.reduce_mean(-tf.compat.v1.log(predict_fake + EPS))
        gen_loss_L1 = tf.compat.v1.reduce_mean(tf.compat.v1.abs(targets - outputs))
    ...
    with tf.compat.v1.name_scope("generator_train"):
        with tf.compat.v1.control_dependencies([discrim_train]):
    ...
    return Model(
        ...
        outputs=outputs,
        train=tf.compat.v1.group(update_losses, incr_global_step, gen_train),
    )
...
def main():
    ...
    if a.mode == "export":
        ...
        batch_input = tf.compat.v1.expand_dims(input_image, axis=0)

        with tf.compat.v1.variable_scope("generator"):
            batch_output = deprocess(create_generator(preprocess(batch_input), 3))

        output_image = tf.compat.v1.image.convert_image_dtype(batch_output, dtype=tf.compat.v1.uint8)[0]
        if a.output_filetype == "png":
            output_data = tf.compat.v1.image.encode_png(output_image)
        elif a.output_filetype == "jpeg":
            output_data = tf.compat.v1.image.encode_jpeg(output_image, quality=80)
        else:
            raise Exception("invalid filetype")
        output = tf.compat.v1.convert_to_tensor([tf.compat.v1.encode_base64(output_data)])
    ...
    def convert(image):
        if a.aspect_ratio != 1.0:
            ...
            image = tf.compat.v1.image.resize_images(image, size=size, method=tf.compat.v1.image.ResizeMethod.BICUBIC)

        return tf.compat.v1.image.convert_image_dtype(image, dtype=tf.compat.v1.uint8, saturate=True)
    ...
    with tf.compat.v1.name_scope("encode_images"):
        display_fetches = {
            ...
            "inputs": tf.compat.v1.map_fn(tf.compat.v1.image.encode_png, converted_inputs, dtype=tf.compat.v1.string, name="input_pngs"),
            "targets": tf.compat.v1.map_fn(tf.compat.v1.image.encode_png, converted_targets, dtype=tf.compat.v1.string, name="target_pngs"),
            "outputs": tf.compat.v1.map_fn(tf.compat.v1.image.encode_png, converted_outputs, dtype=tf.compat.v1.string, name="output_pngs"),
        }
    ...
    with tf.compat.v1.name_scope("predict_real_summary"):
        tf.compat.v1.summary.image("predict_real", tf.compat.v1.image.convert_image_dtype(model.predict_real, dtype=tf.compat.v1.uint8))

    with tf.compat.v1.name_scope("predict_fake_summary"):
        tf.compat.v1.summary.image("predict_fake", tf.compat.v1.image.convert_image_dtype(model.predict_fake, dtype=tf.compat.v1.uint8))
    ...
    with tf.compat.v1.name_scope("parameter_count"):
        parameter_count = tf.compat.v1.reduce_sum([tf.compat.v1.reduce_prod(tf.compat.v1.shape(v)) for v in tf.compat.v1.trainable_variables()])
