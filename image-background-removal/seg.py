...
def drawSegment(baseImg, matImg, filename):
    ...
    img.save(outputFilePath + '/' + filename + '.png')
...
def run_visualization(filepath):
    for filename in os.listdir(filepath):
        ...
        try:
            ...
            # f = open(sys.argv[1])
            jpeg_str = open(fullpath, "rb").read()
            orignal_im = Image.open(BytesIO(jpeg_str))
        except IOError:
            print('Cannot retrieve image. Please check file: ' + fullpath)
            return

        print('running deeplab on image %s...' % fullpath)
